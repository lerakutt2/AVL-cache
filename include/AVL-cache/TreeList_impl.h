...
void TreeList<ReturnType, InputTypes...>::CreateTrees() {
  ...
  [&]<size_t... Is>(std::index_sequence<Is...>) { (CreateTreeList<Is>(), ...); }
...
void TreeList<ReturnType, InputTypes...>::CreateTreeList() {
  treelist.push_back(
      PossibleTypes{std::in_place_index<idx>, &std::get<idx>(trees)});
}

template <typename ReturnType, typename... InputTypes>
template <size_t idx>
void TreeList<ReturnType, InputTypes...>::SetValues(
    const std::tuple<InputTypes...> &data) {
...
ReturnType TreeList<ReturnType, InputTypes...>::GetValue(InputTypes... params) {
  ...
  [&]<size_t... Is>(std::index_sequence<Is...>) {
    ((Is < paramsCount ? SetValues<Is>(paramsTuple) : void()), ...);
  }
  (std::index_sequence_for<InputTypes...>{});

  auto *it = treelist.head.get(); // iterator

  void *currHead = static_cast<void *>(
      &head->next); // remembering the head of the first tree
  void *currNode = static_cast<void *>(head->next); // leaf to search from

  // searching in every tree
  while (it) {
    // searching in tree on some level, FindAbstract returns a pointer to the
    // leaf on a NEXT level (head of next tree)
    currNode = std::visit(
        [&](auto &tree) -> void * { return tree->FindAbstract(currNode); },
        it->tree);

    if (currNode == nullptr)
      break; // node is not found on one of the trees, need to create

    if (!it->next.get())
      break; // node is found and this is the last tree (the next tree doesn't
             // exist)
  ...
  void *val = nullptr;
  if (currNode == nullptr) { // node is not found.
                             // the iterator is still on a tree that misses the
                             // element. so we need to create leaf on this tree
                             // and next trees and connect them.
    ...
    currNode = std::visit(
        ...
        it->tree); // found a place for a new node, created it, returned
                   // node->next (head of a next tree that is yet nullptr)

    while (it->next.get()) {
      it = it->next.get(); // moving to the next tree

      currNode = std::visit(
          [&](auto &tree) -> void * { return tree->NewNodeAbstract(currNode); },
          it->tree);
    }
